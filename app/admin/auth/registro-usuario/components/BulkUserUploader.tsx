...
export function BulkUserUploader() {
  ...
  const processFile = (file: File) => {
    ...
    if (!file.name.endsWith(".xlsx") && !file.name.endsWith(".xls")) {
      ...
      toast.error("Archivo no soportado. Usa .xlsx o .xls")
      return
    }
  ...
  const validateUsersWithServer = async (input: BulkUser[]) => {
    ...
    const localValidated = input.map((user) => {
      ...
      if (user.licenciatura !== "CONTADOR_PUBLICO" && user.licenciatura !== "ADMINISTRACION" && user.licenciatura !== "INFORMATICA" && user.licenciatura !== "ADMINISTRACION_PUBLICA" && user.licenciatura !== "ECONOMIA" && user.licenciatura !== "ADMINISTRACION_POLITICAS_PUBLICAS" && user.licenciatura !== "NO_APLICA") {
        return { ...user, error: "Tipo de licenciatura inválida" }
      }
      if (!user.password) {
        return { ...user, error: "Contraseña faltante" }
      }
              ...
              {!fileName ? (
                <>
                  ...
                  <div>
                    ...
                    <p className="mb-4 text-gray-600">o haz clic para seleccionar un archivo .xlsx o .xls - Excel</p>
                    <Input
                      type="file"
                      accept=".xlsx, .xls"
                ...
                <>
                  ...
                  {fileName && isInvalidFile && (
                    <div className="duration-500 animate-in fade-in-50">
                      ...
                      <p className="mb-4 text-sm text-red-600">Solo se aceptan archivos .xlsx o .xls</p>
                      <Button
                        variant="destructive"
                        onClick={handleRemoveFile}
                        className="transition-transform hover:scale-105"
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {users.map((user, i) => (
                    <tr
                      key={i}
                      className={`border-b transition-all duration-200 hover:bg-gray-50 ${user.error ? "border-red-500 bg-red-200 hover:bg-red-100" : "border-green-200 bg-green-50"} ${editingIndex === i ? "bg-blue-50 ring-2 ring-blue-200" : ""} `}
                    >
                      {editingIndex === i ? (
                        <>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            {user.error ? (
                              <Badge variant="destructive" className="text-xs w-[150px] text-center">
                                {user.error}
                              </Badge>
                            ) : (
                        ...
                        <>
                          ...
                          <td className="p-4">
                            <Badge variant="outline" className="text-xs hover:border-blue-300 hover:bg-blue-50">
                          ...
                          <td className="p-4">
                            {user.error ? (
                              ...
                              <Badge variant="default" className="bg-green-100 text-xs text-green-800 hover:border-green-300 hover:bg-green-50">
